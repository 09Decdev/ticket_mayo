...
export class DistributionService {
  ...
  /**
   * Phục dựng payload email của 1 vé theo claimToken — dùng cho endpoint
   * "Tải vé PDF" trong email. Cùng nguồn dữ liệu với resendEmails: PreTicket
   * (claimToken/job) + DistributionJob (snapshot event/ticketType) + PortalUser
   * (email thật/displayName) + TicketType tươi từ content (eventDate/venue/ảnh —
   * presigned URL trả lại mới nên banner fetch được ngay cả khi URL cũ hết hạn).
   * Trả null khi token/job không còn — controller trả 404.
   */
  async buildPayloadByClaimToken(claimToken: string): Promise<ClaimMailPayload | null> {
    ...
    const job = await this.prisma.distributionJob.findUnique({
      where: { id: preTicket.jobId },
    });
    if (!job) return null;

    const user = await this.prisma.portalUser.findUnique({
      where: { emailHash: preTicket.recipientEmailHash },
      select: { email: true, displayName: true },
    });

    // TicketType/event tươi từ content-service; content lỗi → payload vẫn đủ
    // để in PDF (thiếu ngày/địa điểm/ảnh), không fail cả lần tải.
    let tt: Awaited<ReturnType<EventService['getTicketTypeWithEvent']>> | null = null;
    try {
      tt = await this.eventService.getTicketTypeWithEvent(job.ticketTypeId);
    } catch (err) {
      this.logger.warn(
        `[PDF] getTicketTypeWithEvent lỗi token=${claimToken}: ${(err as Error).message}`,
      );
    }

    // Tên chữ ký — CÙNG nguồn với luồng generate email (distribute): lookup
    // user-community (SOT tên người dùng app), KHÔNG lấy portalUser.displayName
    // làm nguồn chính (đó là tên tự đăng ký portal, có thể sai như "API Test").
    // Fail-soft: lookup lỗi → fallback displayName rồi tên mặc định, không fail PDF.
    let customerName: string | null = null;
    if (user?.email) {
      const nameMap = await this.userCommunity.lookupDisplayNames([user.email]);
      customerName = nameMap.get(normalizeEmailForLookup(user.email)) ?? null;
    }
    if (!customerName) {
    ...
    return {
      jobId: job.id,
      email: user?.email ?? '',
      claimToken: preTicket.claimToken,
      claimUrl: this.mailDispatcher.buildClaimUrl(preTicket.claimToken),
      ...
      customerPhone: '',
      bookedAt: this.formatDateTime(job.createdAt),
      ticketCode: preTicket.contentTicketCode ?? preTicket.claimToken.slice(-8).toUpperCase(),
      ticketId: preTicket.contentTicketId ?? undefined,
    };
  }
